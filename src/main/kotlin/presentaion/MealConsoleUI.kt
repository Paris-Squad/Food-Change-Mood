...
//    2- Enable meal search by mealName. Keep in mind that users may not remember the full meal mealName. Using .contains() may fail if there is a typo in the keyword. Also, due to the large dataset, choose a fast text search algorithm. I recommend the Knuth-Morris-Pratt algorithm for its speed, but feel free to use a better alternative in terms of both performance and accuracy.
...
//    5- Guess Game: Show the user a random meal mealName and ask them to guess its preparation time. The user has 3 attempts. After each attempt, indicate whether the guessed time is correct, too low, or too high. If all attempts are incorrect, show the correct time.
//    6- Sweets with No Eggs: For users allergic to eggs, suggest one sweet (mealName and description) that contains no eggs. The user can either like it (to view full details) or dislike it (to get another random egg-free sweet). Ensure the same sweet is not suggested more than once.
...
//    10- Explore Other Countries' Food Culture: Let the user enter a country mealName, then search any relevant column to return up to 20 randomly ordered meals related to that country.
//    11- Ingredient Game: Display a meal mealName and three ingredient options (one correct, two incorrect). The user guesses once. A correct guess earns 1000 points; an incorrect guess ends the game. The game also ends after 15 correct answers. Display the final score at the end.
...
//    14- Show a list of all seafood meals sorted by protein content, from highest to lowest. Each result should display the rank (starting from 1), meal mealName, and protein amount.
